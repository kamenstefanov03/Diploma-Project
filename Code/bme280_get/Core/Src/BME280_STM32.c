...
void TrimRead(void)
{
	...
	HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, 0x88, 1, trimdata, 25, HAL_MAX_DELAY);

	// Read NVM from 0xE1 to 0xE7
	HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, 0xE1, 1, (uint8_t *)trimdata+25, 7, HAL_MAX_DELAY);
...
int BME280_Config (uint8_t osrs_t, uint8_t osrs_p, uint8_t osrs_h, uint8_t mode, uint8_t t_sb, uint8_t filter)
{
	...
	if (HAL_SPI_Mem_Write(BME280_SPI, BME280_ADDRESS, RESET_REG, 1, &datatowrite, 1, 1000) != HAL_OK)
	{
		return -1;
	}
	...
	datatowrite = osrs_h;
	if (HAL_SPI_Mem_Write(BME280_SPI, BME280_ADDRESS, CTRL_HUM_REG, 1, &datatowrite, 1, 1000) != HAL_OK)
	{
		return -1;
	}
	HAL_Delay (100);
	HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, CTRL_HUM_REG, 1, &datacheck, 1, 1000);
	...
	if (HAL_SPI_Mem_Write(BME280_SPI, BME280_ADDRESS, CONFIG_REG, 1, &datatowrite, 1, 1000) != HAL_OK)
	{
		return -1;
	}
	HAL_Delay (100);
	HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, CONFIG_REG, 1, &datacheck, 1, 1000);
	if (datacheck != datatowrite)
	{
		return -1;
	}
	...
	if (HAL_SPI_Mem_Write(BME280_SPI, BME280_ADDRESS, CTRL_MEAS_REG, 1, &datatowrite, 1, 1000) != HAL_OK)
	{
		return -1;
	}
	HAL_Delay (100);
	HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, CTRL_MEAS_REG, 1, &datacheck, 1, 1000);
	if (datacheck != datatowrite)
	{
		return -1;
	}
...
int BMEReadRaw(void)
{
	...
	HAL_SPI_Mem_Read(&hspi1, BME280_ADDRESS, ID_REG, 1, &chipID, 1, 1000);

	if (chipID == 0x60)
	{
		// Read the Registers 0xF7 to 0xFE
		HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, PRESS_MSB_REG, 1, RawData, 8, HAL_MAX_DELAY);
...
void BME280_WakeUP(void)
{
	...
	HAL_SPI_Mem_Read(BME280_SPI, BME280_ADDRESS, CTRL_MEAS_REG, 1, &datatowrite, 1, 1000);

	// modify the data with the forced mode
	datatowrite = datatowrite | MODE_FORCED;

	// write the new data to the register
	HAL_SPI_Mem_Write(BME280_SPI, BME280_ADDRESS, CTRL_MEAS_REG, 1, &datatowrite, 1, 1000);
